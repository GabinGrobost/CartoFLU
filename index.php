...
let trackedAircraftPopupOpen = false;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  applyRuntimeConfig();
  if (runtimeConfig.showAllAircraft) startFlightRadarTracking();
  refreshBasemapSelectOptions();
...
function normalizeAircraftPoints(points = []) {
  ...
  return points
    .map(p => {
      const lat = Number(p?.lat ?? p?.latitude);
      const lon = Number(p?.lon ?? p?.lng ?? p?.longitude);
      const registration = String(p?.registration || '').trim().toUpperCase();
      const callsign = String(p?.callsign || '').trim().toUpperCase();
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;
      return { lat, lon, registration, callsign };
    })
    .filter(Boolean);
}

function computeHeadingFromTrack(points = []) {
...
function isRotorcraft(flightData = {}) {
  const hints = [
    ...
    flightData.aircraftDescription,
    flightData.aircraftCategory,
    flightData.meta?.aircraftType,
    flightData.meta?.aircraftDescription
  ]
    .map(v => String(v || '').toLowerCase())
    .join(' ');
  return /(heli|helico|hélico|rotor|rotary|ec\d{2,3}|as\d{2,3}|h1[0-9]{2}|r44|r66|bk117|aw\d{2,3})/.test(hints);
}

function formatFlightRadarPopupDetails(flightData = {}, normalizedReg = '', currentLat = null, currentLon = null) {
  const details = [
    ['Immatriculation', normalizedReg],
    ['Vol', String(flightData.callsign || flightData.meta?.callsign || '').trim().toUpperCase()],
    ['ICAO24', String(flightData.icao24 || flightData.meta?.icao24 || '').trim().toUpperCase()],
    ['Type appareil', flightData.aircraftType || flightData.meta?.aircraftType || ''],
    ['Description', flightData.aircraftDescription || flightData.meta?.aircraftDescription || ''],
    ['Catégorie', flightData.aircraftCategory || ''],
    ['Opérateur', flightData.operator || ''],
    ['Origine', flightData.origin || ''],
    ['Destination', flightData.destination || ''],
    ['Squawk', flightData.squawk ?? flightData.meta?.squawk ?? ''],
...
function normalizeFlightRadarSnapshot(snapshot = {}) {
  ...
  return {
    ...
    allAircraft,
    callsign: String(snapshot.callsign || '').trim().toUpperCase(),
    icao24: String(snapshot.icao24 || '').trim().toLowerCase(),
    ...
    aircraftDescription: snapshot.aircraftDescription || '',
    aircraftCategory: snapshot.aircraftCategory || '',
...
function stopFlightRadarTracking() {
  ...
  trackedAircraftBreadcrumbs = [];
  trackedAircraftRegistration = '';
  trackedAircraftPopupOpen = false;
  clearFlightRadarOverlay();
}

function drawFlightRadarOverlay(flightData = {}, registration = '') {
  const hadPopupOpen = trackedAircraftPopupOpen || !!currentFlightRadarState?.popupOpen || !!flightData.popupOpen;
  clearFlightRadarOverlay();
  ...
  if (allAircraft.length > 0) {
    const icon = L.divIcon({
      className: 'plane-snapshot-icon',
      html: '<div style="font-size:13px;line-height:1;opacity:.9;filter:drop-shadow(0 0 1px #000)">✈</div>',
      iconSize: [14, 14],
      iconAnchor: [7, 7]
    });
    allAircraft.forEach(ac => {
      const label = ac.registration || ac.callsign || 'Avion';
      L.marker([ac.lat, ac.lon], { icon })
        .bindPopup(`<div class="popup-title">✈ ${escapeHtml(label)}</div>`)
  ...
  const popupBody = `
    <div class="popup-title">${escapeHtml(iconGlyph)} ${escapeHtml(normalizedReg || 'Appareil')}</div>
  ...
  if (!radarLost && Number.isFinite(currentLat) && Number.isFinite(currentLon)) {
    ...
    const trackedMarker = L.marker([currentLat, currentLon], { icon: planeIcon })
      ...
      .addTo(flightradarLayer);
    trackedMarker.on('popupopen', () => { trackedAircraftPopupOpen = true; });
    trackedMarker.on('popupclose', () => { trackedAircraftPopupOpen = false; });
    if (hadPopupOpen) trackedMarker.openPopup();
  }
  ...
  currentFlightRadarState = normalizeFlightRadarSnapshot({
    ...
    squawk: flightData.squawk ?? null,
    aircraftType: flightData.aircraftType || '',
    aircraftDescription: flightData.aircraftDescription || '',
    aircraftCategory: flightData.aircraftCategory || '',
...
function normalizeFlightradarResponse(raw = {}) {
  ...
  return {
    ...
    squawk: data?.squawk ?? data?.meta?.squawk ?? null,
    aircraftType: data?.aircraftType ?? data?.meta?.aircraftType ?? '',
    aircraftDescription: data?.aircraftDescription ?? data?.meta?.aircraftDescription ?? '',
    aircraftCategory: data?.aircraftCategory ?? '',
...
async function fetchFlightRadarTrack(registration = '', options = {}) {
  const reg = String(registration || '').trim().toUpperCase();
  const all = !!options.all;
  if (!all && !reg) return null;
...
function startFlightRadarTracking() {
  ...
  const refreshMs = isMonoAircraftTracking
    ? Math.max(300, Math.min(1000, Number(runtimeConfig.flightradar24RefreshMs) || 1000))
    ...
    list.innerHTML = archived.map(op => {
      ...
      return `<div class="sar-archived-row">
        ...
        <div class="sar-archived-actions">
          <button type="button" class="btn btn-secondary sar-archived-export-btn" title="Exporter les relevés de l'archive au format XLSX" onclick="exportArchivedOperationXlsx('${ref}')"><svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M17 17H17.01M17.4 14H18C18.9319 14 19.3978 14 19.7654 14.1522C20.2554 14.3552 20.6448 14.7446 20.8478 15.2346C21 15.6022 21 16.0681 21 17C21 17.9319 21 18.3978 20.8478 18.7654C20.6448 19.2554 20.2554 19.6448 19.7654 19.8478C19.3978 20 18.9319 20 18 20H6C5.06812 20 4.60218 20 4.23463 19.8478C3.74458 19.6448 3.35523 19.2554 3.15224 18.7654C3 18.3978 3 17.9319 3 17C3 16.0681 3 15.6022 3.15224 15.2346C3.35523 14.7446 3.74458 14.3552 4.23463 14.1522C4.60218 14 5.06812 14 6 14H6.6M12 15V4M12 15L9 12M12 15L15 12" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg> Exporter XLSX</button>
          <button type="button" class="btn btn-secondary sar-archived-export-btn" title="Exporter les relevés de l'archive au format PDF (A4)" onclick="exportArchivedOperationPdf('${ref}')"><svg width="25px" height="25px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M17 17H17.01M17.4 14H18C18.9319 14 19.3978 14 19.7654 14.1522C20.2554 14.3552 20.6448 14.7446 20.8478 15.2346C21 15.6022 21 16.0681 21 17C21 17.9319 21 18.3978 20.8478 18.7654C20.6448 19.2554 20.2554 19.6448 19.7654 19.8478C19.3978 20 18.9319 20 18 20H6C5.06812 20 4.60218 20 4.23463 19.8478C3.74458 19.6448 3.35523 19.2554 3.15224 18.7654C3 18.3978 3 17.9319 3 17C3 16.0681 3 15.6022 3.15224 15.2346C3.35523 14.7446 3.74458 14.3552 4.23463 14.1522C4.60218 14 5.06812 14 6 14H6.6M12 15V4M12 15L9 12M12 15L15 12" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg> Exporter PDF</button>
...
async function closeOperation() {
  ...
  if (!runtimeConfig.showAllAircraft) stopFlightRadarTracking();
  closeSarOperationOverlay();
  closeSarOfflineModal();
  closeArchivedOperationsModal();
  closePcMenu();
